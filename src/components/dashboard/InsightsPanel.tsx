...
export default function InsightsPanel({ insights, alertas, processingTimeMs }: InsightsPanelProps) {
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative h-full">
      {processingTimeMs && processingTimeMs > 0 && (
        <div className="absolute top-5 right-5 flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 bg-slate-100 text-slate-500 rounded-lg">
          <Clock size={14}/> {(processingTimeMs / 1000).toFixed(2)}s
        </div>
      )}
      
      <h3 className="text-lg font-bold border-b border-slate-100 pb-3 mb-4 flex items-center gap-2 text-slate-800">
        <FileText size={20} className="text-slate-400"/> Insights da IA
      </h3>
